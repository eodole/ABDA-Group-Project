...
for (i in seq_along(df_plot[,vars_to_plot])) {
  ...
  plot <- ggplot(df_plot, aes_string(x = variable_name)) +
    ...
    scale_x_continuous(labels = comma, breaks = pretty(df_plot$variable_name)) +
    theme_minimal() +
    theme(
...
winner_barchart <- ggplot(df_plot, aes(x = factor(Winning.party))) +
                    geom_bar(width = 0.2 ,fill = c("red", "blue"), alpha = 0.7, color = "black") +  # Custom colors
                    labs(title = "Winning.party", x = "Party", y = "Frequency") +
...
# Reorder the urban.cat variable in df_plot
df_plot$urban.cat <- factor(df_plot$urban.cat, levels = categ_order)
  
  
# Urban Category

# Create the bar chart with a custom gradient based on order
urbcat_barchart <- ggplot(df_plot, aes(x = factor(urban.cat), fill = factor(urban.cat))) +
                    geom_bar(width = 0.2, alpha = 0.7, color = "black") +  # Custom fill colors
                    labs(title = "Urban Category", x = "Category", y = "Frequency") +
                    scale_x_discrete() +
                    scale_fill_manual(values = scales::gradient_n_pal(c("darkgreen", "white"))(
                      seq(0, 1, length.out = length(unique(df_plot$urban.cat)))
                    )) +  # Gradient based on order
                    theme_minimal() +
                    theme(
                      plot.title = element_text(size = 30),  # Adjust title size and make it bold
                      axis.title = element_text(size = 25),  # Adjust axis title size
                      axis.text = element_text(size = 25, angle = 45, hjust = 1, vjust = 0.5),  # Rotate x-axis text
...
winning_party_data <- data.frame(
  district = df_plot$stcd,  # State-District format
  Winning.party = factor(df_plot$Winning.party, levels = c(0, 1), labels = c("Republican", "Democratic"))  # Convert to factor
)
