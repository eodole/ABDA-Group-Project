---
title: "joindfs"
output: html_document
date: "2025-01-04"
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```

```{r}
# Load required package
library(dplyr)
library(stringr)
```


```{r}
setwd("C:/Users/HP/Desktop/Msc Econometrics/ABDA/project")


# Read the CSV files
df_elect_res <- read.csv("election_results_house.csv")
df_races <- read.csv("races.csv")
df_urb_index <- read.csv("urbanization-index-2022.csv")

head(df_elect_res)
head(df_races)
head(df_urb_index)

```




```{r}
# Join df_elect_res and df_races, by race id

# adjust column names

df_races2 <- df_races %>% 
  rename(
    race_id = id 
    )


# inner_join because we only need the house races

joined1 <- df_elect_res %>%
  inner_join(df_races2, by = "race_id")

# Drop identical columns
df1 <- joined1 %>%
  select(-one_of(names(joined1)[sapply(names(joined1), function(col) {
    if (col %in% c("state", "district")) return(FALSE)
    if (!grepl("\\.x$", col)) return(FALSE)
    corresponding_col <- sub("\\.x$", ".y", col)
    corresponding_col %in% names(joined1) && all(joined1[[col]] == joined1[[corresponding_col]], na.rm = TRUE)
  })]))


# View the resulting dataset
head(df1)

```



```{r}

# Join the datasets by 'state' and 'district'

# Adjust district code and col names

df2 <- df1 %>%
  mutate(district = as.double(str_remove(office_seat_name.y, "^District ")), state_code = state_abbrev.y) 

df3 <- df_urb_index %>%
  rename(
    district = cd,
    state_code = state
  )



# Adjust the type of join (inner_join, left_join, etc.) as needed
df_all_vars <- df3 %>%
  inner_join(df2, by = c("state_code", "district")) 

# View the resulting dataset
head(df_all_vars)


# Count unique values in each column
unique_counts <- df_all_vars %>%
  summarise(across(everything(), ~ n_distinct(.)))

# View the counts as a dataframe
unique_counts


```


```{r}



df_all_vars_2022 <- df_all_vars %>%
  filter(cycle.y == 2022)



# View the resulting dataset
head(df_all_vars_2022)


# Count unique values in each column
unique_counts <- df_all_vars_2022 %>%
  summarise(across(everything(), ~ n_distinct(.)))

# View the counts as a dataframe
unique_counts


```



```{r}

df_demographics <- read.csv("us_demographic_data_clean.csv")


head(df_demographics)

# Adjust the variables for joining

df_demographics <- df_demographics %>%
  mutate(district = case_when(
    str_detect(Name, "at Large") ~ "1",  # For at-large districts, assign "1"
    TRUE ~ str_extract(Name, "(?<=District )\\d+")  # Extract number after "District"
  ))

df_demographics <- df_demographics %>%
  mutate(district = as.double(district)) 


head(df_demographics)




```


```{r}

# join everything

joined_all <- df_all_vars_2022 %>%
  inner_join(df_demographics, by = c("State", "district")) 


head(joined_all)

```


```{r}

# Define the columns to keep
cols_to_keep <- c("stcd", "state_code","district","urbanindex","grouping","ballot_party","percent","winner","incumbent_party", "Winning.party","Total.Population", "Percentage.Women", "Median.Household.Income", "Mean.Household.Income", "Median.Age", "Percentage.Retirees..65..", "Percentage.Bachelors.Degree.in.Population.over.25", "Unemployment.Rate..16.and.over.")

# Keep only the specified columns
joined_final <- joined_all %>%
  select(all_of(cols_to_keep))


```


```{r}
# Save your dataframe as a CSV file
write.csv(joined_final, "joined_final_with_winner_party.csv", row.names = FALSE)

```



```{r}

```







