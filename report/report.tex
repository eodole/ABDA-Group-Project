...
Figure \ref{fig:rmse} \red{TAKE OUT THAT TITLE} shows the histograms of the RMSE with all (4000)\blue{didn't we increase iterations?} draws for each of our models.
Model 4 estimates on average closer percentages to the actual outcome which, in a certain sense, can mean it estimates \blue{voting outcomes?} better. This is however unsurpising, as this model has many more covariates, and at least some have a significant (?) \red{did you mean a ? here? maybe say large instead of significant not to be confused with statistically signigicant} effect on the probability of a Democrat win.
...
Table \ref{tab:LPD_diff} shows the sum of these differences, i.e., the difference in Log-Pointwise Predictive Density (LPD) between models. 
...
\begin{table}[ht]
	...
	\begin{tabular}{lcc}
		...
		Model 3 - Model 4 &    -17.92 &          3.66 \\ \bottomrule
	\end{tabular}
	\caption{text}
	\label{tab:LPD_diff}
\end{table}
...
According to the LOO statistics (Table \ref{tab:loo}), Model 4 (the largest) is the preferred one, followed by Models 3, 1 and 2. This is the same ranking we had seen before, with the more complex models performing better.
