...
\begin{itemize}
	...
	\item percentage women
	\item median household income
	\item mean household income
	\item percentage retirees
	\item percentage bachelors degree holders above 25 years old
	\item unemployment rate 
	\item region
\end{itemize}
We picked these out of the many possible covariates available in the American Community Survey to give us a wide variety of covariates to work with. In our inital brainstorming, we thought that education, income, as well as demographic make-up might all be influential in partisan voting outcomes, all of our initial covariates were measuring at least one of these aspects. 

To then build some knowledge into our models, we looked into different qualitative studies about voting behavior, which helped guide our final choices of covariates. As previously explained, the districts are drawn in such a way that the total population of each district should be approximately the same, so while this does help support our exchangability assumption, total population itself would be an unsuitable covariate. 

However as we researched more deeply, we found that some association were stronger than others. 
Although we initialally thou
...
\begin{table}[h]
	\resizebox{\textwidth}{!}{%
		\begin{tabular}{l|llll}
			\toprule
			& \multicolumn{1}{c}{Model 3}              & \multicolumn{1}{c}{Model 4}              & \multicolumn{1}{c}{Model 2}                  & \multicolumn{1}{c}{Model 1}                  \\ 
			\hline
			...
			& $\beta_{urb, j} \sim N(0, \sigma_{j}), $ & $\beta_{urb, k} \sim N(0, \sigma_{k}), $ & $\beta_{urb, k} \sim N(0, \sigma_{k}), $     & $\beta_{urb, j} \sim N(0, \sigma_{urb,j}), $ \\
			& $\sigma_{j} \sim Halfcauchy(10)$         & $\sigma_{k} \sim Halfcauchy(10)$         & $\sigma_{k} \sim Halfcauchy(10)$             & $\sigma_{urb,j} \sim Halfcauchy(10)$             \\
