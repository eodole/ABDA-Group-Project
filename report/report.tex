...
In this report we aim to investigate the relationship between urbanization and partisanship voting outcomes \red{in the US, using Bayesian Multilevel modeling}. Put more concretely we want to answer the question; "How does urbanization of a particular US House district affect the party that is elected?". First we explain our motivation and important background information about the US election system. 
...
We then discuss the set up of our four models in detail, followed by a discussion of the priors and our reasoning behind them. We then go into the results of our models and their convergence diagnostics. Next we use both graphical and numerical methods \red{not graphical, we can just say bayesian methods} to compare our models. Finally we perform a prior sensitivity analysis before concluding with limitations, improvements and reflections. 
...
\red{the bit about which is the progressive party should go here} 

To answer our question about the nature of the relationship between urbanization and partisan voting outcomes, we choose to investigate the 2022 US House Election. Every two years the United States elects 435 officials to the House of Representatives. Each state is allocated one of the the 435 House seats with the rest being allocated roughly proportional to the share of total population living in each state. 
...
%\blue{ Why is pct retirees included in every single one of our models? } \red{I think the previous sentence clears this up enough}
...
%%%Copy this part
\blue{Here we should include a short description of every file that will be in report code that will be turned in with the report for example...} \red{I think instead we should just upload a text document/excel sheet to the zip file explaining what each file is, and mention that there is a description file in the report}
...
In brms the hierarchical coefficients are modelled as a centred Normal with a varying standard deviation, which itself follows a prior distribution given by the user \parencite{brmsbook}. \blue{*} \red{it is in the prior section too, we can just delete it here}
...
%\red{it has the strongest correlation with winning party after urban index, and it's the only one negatively correlated with winner, could we justify it one of those ways?} \blue{Sounds good to me}
...
%\textcolor{red}{is there a reason??? M: Yes, but we'll have to dig deep into the MC Stan Forums and see what Paul wrote. Explaining this is out of the scope tbh. Refer to one of my paras where I mention BRMS is simply less flexible than STAN, I could add a couple lines there}\blue{yes please add those sentences}
...
%%**
Firstly, the number of divergent transitions: There are many more of these for Model 3 in comparison to Model 2. Secondly, the place of occurrence: Model 3's divergent transitions occur throughout the chains, while those in Model 2 seem to be concentrated around the 1000th iteration, while noticeably decreasing towards the 2000th iteration. This suggests that increasing iterations further might help with eventually achieving convergence for Model 2, but is unlikely to help with Model 3. \blue{when you say "might help with" what do you mean? Might help with what exactly? }
%%**
The trace plots of the other models and coefficients (in Appendix) either resemble those of Model 2, in that they have divergent transitions near the 1000th iteration and fewer divergent transitions near the 2000th iteration \blue{how so?}, or have no divergent transitions as is the case with Model 1.

Our key takeaway is the HMC chains have more divergent transitions throughout the chains  \blue{when you say behave strangely what does that mean? try to be specific} for Model 3. \red{Luckily for us, there is one glaring difference between Model 3 and the rest which partially explains this behaviour: Namely, it is} \blue{this is really informal, you could also just write: "This is likely due to Model 3 being .."} This is likely due to Model 3 being the only model which takes into account both a Region-wise hierarchy and a State-wise hierarchy for the same coefficient, with the former being nested in the latter. The other models have at most a single hierarchy per coefficient. 
...
So, we conducted a prior sensitivity analysis, by refitting our models using alternative priors (which also fit our basic model assumptions) and assessing the impact on the estimated coefficients.

For easy comparability and scope, we restrict this section to the analysis of Models 1 to 3.
We tested 5 different Prior Sets, which represent 'changes' relative to the priors we used originally. These changes are as follows (all other priors remain as described in Section \ref{sec:priors}):
...
So, the first Set is just our original set of priors, for comparability. Sets 2-4 represent a single prior that is different relative to the original Set, and Set 5 combines two of those changes, to see how estimates behave when more than one prior is altered.
...
The one in Set 3 is less informative than the original, but still symmetric around zero to relax the assumption of the \textit{Urban Index}'s coefficient being 'small' (between -2 and 2 with 95\% certainty). 
The one in Set 4 was chosen to be \textit{very} weakly informative, to see what happens if we start with the belief that the intercept is nearly as likely to attain very small (or large) values as it is to being close to zero.
